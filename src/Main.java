...
public class Main {
    ...
    public static String handle_opposite_err(String original, String line) {
        ...
        try {
            opposite = negative_comparator(original);
        } catch (Exception e) {
            System.out.printf("Error at line %d:\n%s\nInvalid Comparator\n", line_count, line);
            System.exit(1);
        }
    ...
    public static String handle_new_loop(String[] array, String line) {
        ...
        if (array.length == 1)
            return String.format("loop_%d:", loop_count);
        String opposite = handle_opposite_err(array[2], line);
        return String.format("loop_%d:\ncmp %s, %s\nj%s loop_end_%d", loop_count, array[1], array[3], opposite,
                loop_count);
    }
    ...
    public static String handle_continue_break(String mode, String line) {
        ...
        if (mode.equals("continue"))
            return String.format("jmp loop_%s:", top_parts[2]);
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String parameter : parameters) {
            if (parameter.equals("--disable-warnings") || parameter.equals("-dw"))
                DISABLE_WARNINGS = true;
            else if (parameter.equals("--disable-unsafe") || parameter.equals("-du"))
                DISABLE_UNSAFE = true;
            else {
